...
| D66 | `thin` collapses to one line and one action when every cell is thin, instead of listing a whole weak library | 5.5 | 5.6 | 5.6 owns the thin-library experience |
...
| D68 | A per-campaign secondary view, so one campaign in three thin cells is offered for measurement once rather than three times | 5.5 | 5.6 | The grid is right for "what do I have in Colombia"; the action is per campaign |
