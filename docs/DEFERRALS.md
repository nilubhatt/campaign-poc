...
| D41 | Supersession offered only on evidence the schema actually holds — a non-empty `resolved`, or a record that already carries `supersedes` — never on `closest_precedent`, and with the replaced record named in the label | 5.2 | 6.3 | Both reviewers independently condemned the similarity-based version; 6.3 is where supersession and the prediction loop meet |
...
| C12 | `next_step` prose in warnings, on the upload paths (was D16) | 5.2 — `next_actions` with `{label, tool, prefilled_args}`. **Partial:** `results_may_be_incomplete` still carries the advice as prose, and `prepare_evaluation` has no `next_actions` at all — see D45 |
...
| C14 | Notices were ephemeral, so `gaps()` could not report an upload-time finding once the call returned (was D17) | 5.3 — `campaigns.commentary_checked` is persisted, so "this deck's comments were never read" survives the response that first said it |
| C15 | `results_may_be_incomplete` carried `finish_indexing` as prose, and `prepare_evaluation` returned no actions at all (was D45) | 5.3 — both now carry the action, `consent: "do"` |
