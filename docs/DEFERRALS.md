...
| C12 | `next_step` prose in warnings (was D16) | 5.2 — `next_actions` with `{label, tool, prefilled_args}`; `next_step` survives only where no single tool sits behind the advice |
