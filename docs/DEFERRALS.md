...
| D45 | `results_may_be_incomplete` still carries `finish_indexing` as prose in `next_step`, and `prepare_evaluation` returns no `next_actions` at all, though a single tool sits behind both | 5.2 | 5.3 | `gaps()` is the item that decides what a standing "something is missing" line offers |
