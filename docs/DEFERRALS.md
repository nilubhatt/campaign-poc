...
| D65 | Citation concentration per cell (`cited_share_top`, `never_cited`) from `evaluations.cited_ids` — "one example carrying the weight" is a fact about judgments, and a cell of six measured campaigns where one is cited by every verdict reads `measured` today | 5.5 | 6.6 | Needs evaluation traffic to mean anything; the schema already holds it |
...
| D78 | A finding can evade the citation requirement by calling itself `missing_information`, or by omitting `kind` altogether — which is cheaper, since a finding with no kind needs no citation at all. Expect the same pressure on `guardrail_breach`, which needs a rulebook record the model has to have found by itself | 6.1 | 7.7 | `kind` is model-chosen and no server check can read intent. Measurable, not preventable: 7.7's agreement runs are where a systematic drift towards the uncited kinds would show up, the same way 2.4 counts severity downgrades |
